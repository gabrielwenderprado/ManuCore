...
function overview() {
  ...
  return `${filterBar()}<div class="hero executive-hero"><div><span class="eyebrow"></span><h2>Visão geral da operação</h2><p></p></div><div class="date-pill">Base atualizada · ${esc(DATA.generatedAt || DATA.sourceFile)}</div></div>${metrics()}<div class="dashboard-grid">${riskChart()}${orderPanel()}</div>${planMonthPanel()}`;
}
...
function simulationPurchaseQuantity(row) {
  const orderState = simulationOrderState(row);
  return Math.max(0, n(row.need) + n(row.safety) - n(row.stock) - orderState.total);
}
...
function runWeeklySimulation() {
  ...
  const bindSimulationActions = visibleRows => {
    ...
    document.querySelectorAll('.simulation-purchase-btn').forEach(button => button.onclick = () => {
      ...
      if (!row) return;
      const quantity = simulationPurchaseQuantity(row);
